    ...
    <div className="flex-1 flex flex-col min-w-0 h-full overflow-hidden">
      {selectedProcess ? (
        <div className="flex-1 flex flex-col min-w-0 bg-white h-full overflow-hidden">
          <header className="p-6 lg:p-8 border-b border-stone-100 flex items-center justify-between shrink-0">
            <div className="flex items-center gap-3 lg:gap-4 flex-1 min-w-0">
              <button 
                onClick={() => setSelectedProcessId(null)}
                className="w-10 h-10 bg-stone-50 rounded-xl flex items-center justify-center text-stone-400 hover:text-stone-900 transition-all shrink-0"
              >
                <X size={20} />
              </button>
              <div className="min-w-0">
                <h2 className="text-xl lg:text-2xl font-black text-stone-900 uppercase tracking-tight truncate">{selectedProcess.title}</h2>
          ...
          <div className="flex-1 overflow-y-auto p-6 lg:p-12 custom-scrollbar">
            <div className="max-w-3xl mx-auto space-y-8 lg:space-y-12">
              <div className="bg-primary/5 rounded-[32px] lg:rounded-[40px] p-6 lg:p-10 border border-primary/10">
                 <h3 className="text-xs font-black text-primary uppercase tracking-widest mb-4 flex items-center gap-2">
                   <Info size={14} />
                   Objetivo do Processo
                 </h3>
                 <p className="text-stone-600 text-base lg:text-lg leading-relaxed font-medium italic">
                   "{selectedProcess.description || 'Nenhuma descrição detalhada fornecida.'}"
                 </p>
              </div>

              <div className="relative space-y-8">
                <div className="absolute left-6 lg:left-8 top-8 bottom-8 w-1 bg-gradient-to-b from-primary/20 via-primary/5 to-transparent rounded-full ml-[0.5px]" />
                ...
                {selectedProcess.steps.map((step, index) => (
                  ...
                  >
                    <div className="absolute left-0 top-0 w-12 h-12 lg:w-16 lg:h-16 bg-white border-2 lg:border-4 border-stone-50 rounded-full flex items-center justify-center text-lg lg:text-xl font-black text-primary shadow-xl shadow-primary/10 group-hover:scale-110 transition-transform z-10">
                      {index + 1}
                    </div>
                    
                    <div className="bg-stone-50 border border-stone-100 rounded-[24px] lg:rounded-[32px] p-6 lg:p-8 transition-all hover:bg-white hover:shadow-xl hover:shadow-stone-200/50">
                      <h4 className="text-base lg:text-lg font-black text-stone-900 uppercase tracking-tight mb-2">
                        {step.title}
                      </h4>
                      <p className="text-sm lg:text-base text-stone-500 font-medium leading-relaxed">
                        {step.description}
                      </p>
                    </div>
                  </motion.div>
                ...
                <div className="pl-16 lg:pl-24">
                   <div className="bg-stone-900 border border-stone-800 rounded-[24px] lg:rounded-[32px] p-6 lg:p-8 flex items-center justify-between">
                      <div>
                        <h4 className="text-white font-black uppercase tracking-tight mb-1 text-base lg:text-lg">Fim do Processo</h4>
                        <p className="text-stone-500 text-xs lg:text-sm font-medium">Fluxo concluído com sucesso.</p>
                      </div>
                      <div className="w-10 h-10 lg:w-12 lg:h-12 bg-white/10 rounded-xl lg:rounded-2xl flex items-center justify-center text-white">
                        <GitBranch size={20} />
                      </div>
                   </div>
                </div>
            ...
            {filteredProcesses.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-20 text-stone-300">
                 <div className="w-20 h-20 lg:w-24 lg:h-24 bg-stone-100 rounded-[32px] lg:rounded-[40px] flex items-center justify-center mb-6">
                    <GitBranch className="w-10 h-10 lg:w-12 lg:h-12" strokeWidth={1.5} />
                 </div>
                 <p className="font-black uppercase tracking-widest text-xs">Nenhum fluxo mapeado</p>
                 <button 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 2xl:grid-cols-3 gap-6 lg:gap-8 pb-20">
